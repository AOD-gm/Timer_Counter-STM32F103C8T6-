...
uint32_t alarm_timer_ms = 0; 
uint8_t manual_active = 0; 
uint8_t manual_override_off = 0; 
uint32_t manual_off_until = 0; 
...
void Key_Save_Return(char key) {
    if (key == '*') {
        if (menu == 1) {
            if (mode == 0) menu = 3;      
            else if (mode == 1) { menu = 2; setting = 0; } 
            else if (mode == 2) menu = 4; 
            OLED_Clear();
            ...
            if (choose == 1) {
                ...
                HAL_Delay(1000);
                menu = 2; setting = 4; locate = 0; mode = 0; 
                OLED_Clear();
            }
            else if (choose == 2) {
                DS3231 set_alarm;
                ...
                alarm_set = 1;
                HAL_Delay(1000);
                menu = 2; setting = 0; locate = 0; mode = 0; 
                OLED_Clear();
            }
        }
        else if(menu == 3){
...
void UI_Proccess_Keypad(char key) {
    ...
    if (menu == 0) {
        ...
        if (key == 'A') { mode = 0; menu = 1; OLED_Clear(); } 
        else if (key == 'B') { menu = 2; setting = 0; OLED_Clear(); } 
        return;
    }
    ...
    if(menu == 3){
        if(key == 'A'){ set_mode = 0; OLED_Clear(); }
        else if(key == 'B'){ set_mode = 1; OLED_Clear(); }
        else if(key == 'C'){ set_mode = 2; OLED_Clear(); }
        Key_Save_Return(key);
        return;
    }
    ...
    switch (menu) {
        ...
        case 2: 
            if (setting == 0) {
                ...
                char buf_day[20], buf_d[20], buf_m[20], buf_y[20];
                ...
                sprintf(buf_h, "%s GIO  : %02d", (setting == 1 ? "*" : " "), temp_hour);
                sprintf(buf_m, "%s PHUT : %02d", (setting == 2 ? "*" : " "), temp_minute);
                sprintf(buf_s, "%s GIAY : %02d", (setting == 3 ? "*" : " "), temp_second);
...
void UI_ESP_Process(void) {
    if(Rx_Flag)
    {
        ...
        else if(strncmp((char*)u8_RxBuff, "MODE:", 5) == 0)
        {
            if(strstr((char*)u8_RxBuff, "OPTIMISE") != NULL)
            {
                set_mode = 0; 
                OLED_Clear();
                OLED_Print("Mode: Optimise", 10, 3);
            }
            else if(strstr((char*)u8_RxBuff, "NORMAL") != NULL)
            {
                set_mode = 1;
                OLED_Clear();
                OLED_Print("Mode: Normal", 10, 3);
            }
            else if(strstr((char*)u8_RxBuff, "SAVING") != NULL)
            {
                set_mode = 2;
                OLED_Clear();
                OLED_Print("Mode: Saving", 10, 3);
            }
            HAL_Delay(500);
            OLED_Clear();
        }
